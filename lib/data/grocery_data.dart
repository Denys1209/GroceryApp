class GroceryData {
  static List<Map<String, dynamic>> groceryProducts = [
    {
      'id': '1',
      'name': 'Apple',
      'description': 'Fruits',
      'price': 0.5,
      'imageUrl':
          'https://www.shutterstock.com/image-photo/bunch-bananas-isolated-on-white-600nw-1722111529.jpg'
    },
    {
      'id': '2',
      'name': 'Bread',
      'description': 'Bakery',
      'price': 1.5,
      'imageUrl':
          'https://assets.bonappetit.com/photos/5c62e4a3e81bbf522a9579ce/16:9/w_4000,h_2250,c_limit/milk-bread.jpg'
    },
    {
      'id': '3',
      'name': 'Milk',
      'description': 'Dairy',
      'price': 2.0,
      'imageUrl':
          'https://drinkmilk.co.uk/wp-content/uploads/2020/05/milk_2pint.png'
    },
    {
      'id': '4',
      'name': 'Eggs',
      'description': 'Dairy',
      'price': 0.75,
      'imageUrl':
          'https://cdn.britannica.com/94/151894-050-F72A5317/Brown-eggs.jpg'
    },
    {
      'id': '5',
      'name': 'Orange',
      'description': 'Fruits',
      'price': 0.6,
      'imageUrl':
          'https://media.istockphoto.com/id/185284489/photo/orange.jpg?s=612x612&w=0&k=20&c=m4EXknC74i2aYWCbjxbzZ6EtRaJkdSJNtekh4m1PspE='
    },
    {
      'id': '6',
      'name': 'Butter',
      'description': 'Dairy',
      'price': 1.0,
      'imageUrl':
          'https://www.realsimple.com/thmb/VvdPHiBwtcQgPl8MiRbOjSjNo4g=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/freeze-butter-GettyImages-466938239-b386cf1b961642089337ab851e40a87e.jpg'
    },
    {
      'id': '7',
      'name': 'Cheese',
      'description': 'Dairy',
      'price': 3.0,
      'imageUrl':
          'https://ca-times.brightspotcdn.com/dims4/default/54847e8/2147483647/strip/true/crop/2048x1280+0+43/resize/1200x750!/quality/75/?url=https%3A%2F%2Fcalifornia-times-brightspot.s3.amazonaws.com%2Fb2%2Fa5%2Fd673ffac73e3ff63f2f3c095fde9%2Fhomemade-american-cheese-recipes-db'
    },
    {
      'id': '8',
      'name': 'Chicken',
      'description': 'Meat',
      'price': 5.0,
      'imageUrl':
          'https://assets.farmison.com/images/recipe-detail-1380/76340-yorkshire-free-range-loose-birds-chicken3c3e9dd7-8edd-41fa-b35d-73f3cb951b8asquare900x900.jpg'
    },
    {
      'id': '9',
      'name': 'Beans',
      'description': 'Vegetables',
      'price': 0.8,
      'imageUrl':
          'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTsO0SCfUF1sG9GHk2aYKcaO-x1WuRu_Lc61g&usqp=CAU'
    },
    {
      'id': '10',
      'name': 'Carrots',
      'description': 'Vegetables',
      'price': 0.6,
      'imageUrl':
          'https://www.melissas.com/cdn/shop/products/image-of-organic-carrots-organics-28658386305068_600x600.jpg?v=1628076271'
    },
  ];
}
